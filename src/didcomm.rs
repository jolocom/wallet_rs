...
use universal_wallet::{
    didcomm_rs::{crypto::CryptoAlgorithm, Message},
    unlocked::*,
};
...
pub(crate) fn create_message(env: Env) -> ContextlessResult<JsString> {
    env.create_string(&UnlockedWallet::create_message())
        .map(Some)
}
...
fn create_jwe(ctx: CallContext, alg: CryptoAlgorithm) -> Result<JsString> {
    ...
    ctx.env.create_string(&UnlockedWallet::create_jwe_message(
        from.as_str()?,
        &to,
        alg,
    ))
...
pub(crate) fn seal_encrypted(ctx: CallContext) -> Result<JsString> {
    ...
    ctx.env.create_string(
        &wallet
            .unlocked
            .seal_encrypted(message.to_owned())
...
pub(crate) fn seal_encrypted_str(ctx: CallContext) -> Result<JsString> {
    ...
    ctx.env.create_string(
        &wallet
            .unlocked
            .seal_encrypted_str(message.as_str()?)
            .map_err(|e| napi::Error::from_reason(e.to_string()))?,
    )
}

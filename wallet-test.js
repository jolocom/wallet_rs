...
const { attach, detach } = require('./index.js')
...
test('attach wallet from native code', (t) => {
    t.notThrows(() => alice_wallet.att(alice_string, "alice", Buffer.from("alice", "utf8").toString('base64')))
})

test('sleep function from native code', (t) => {
    t.notThrows(() => {
        const pass = Buffer.from("alice", 'utf-8').toString('base64')
        alice_wallet.det(pass)
    })
})
...
test('return of encrypted matches old', (t) => {
    const pass = Buffer.from("alice", "utf8").toString('base64')
    alice_wallet2.att(alice_string, "alice", pass)
    const new_alice = alice_wallet2.det(pass)
    t.deepEqual(new_alice, alice_string)
})
